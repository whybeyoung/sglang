    ...
    def get_wrs_batch_notwork(self, mrs, mrs_info, remote_mrs_info):
        ...
        for layer_id, layer_base in enumerate(remote_mrs_info.get("layer_base_addrs", [])):
            for index, offset in enumerate(remote_mrs_info.get("offsets")):
                ...
                offset_batch.append(offset)  # 记录当前 SGE 的 offset
                ...
                if len(sge_batch) == 30:
                    ...
                    wr = SendWR(
                        ...
                        sg=sge_batch[:],  # 复制 SGE 列表
                        num_sge=len(sge_batch),
                        opcode=IBV_WR_RDMA_WRITE,
                        send_flags=IBV_SEND_SIGNALED
                    )
                    wr.set_wr_rdma(
                        addr=layer_base + offset_batch[0],  # 以第一个 SGE 的 offset 作为 WR 的基址
                        rkey=remote_mrs_info.get("rkeys")[layer_id]
                    )
                    self.wrs_to_send.append(wr)
                    sge_batch.clear()  # 清空桶
                    offset_batch.clear()  # 清空 offset 记录
        ...
        if sge_batch:
            ...
            wr.set_wr_rdma(
                addr=layer_base + offset_batch[0],  # 取第一个 SGE 的 offset 作为 WR 基址
                rkey=remote_mrs_info.get("rkeys")[layer_id]
            )
            self.wrs_to_send.append(wr)
