...
def _test_peer_communication_rma(queue, rank, msg_size):
    ...
    print(f"rank{rank} worker address: ", self_address)
    ...
    if rank == 0:
        print("client: mem handle: ", mem_handle)
    else:
        print("server: mem handle: ", mem_handle)
    ...
    if rank == 0:
        ...
        print("client: mem : ", self_mem)
    else:
        print("server: mem : ", self_mem)
...
@pytest.mark.parametrize(
    "test_name", [_test_peer_communication_rma]
)
@pytest.mark.parametrize("msg_size", [ 2**24])
