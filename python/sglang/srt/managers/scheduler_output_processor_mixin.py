...
from sglang.srt.managers.utils import RandomNumberGenerator
if TYPE_CHECKING:
...
class SchedulerOutputProcessorMixin:
    ...
    def stream_output(
        ...
        if self.server_args.detokenizer_worker_num > 1:
            workeridReqsMap = {}
            for req in reqs:
                worker_id = get_workerids_from_rids(req.rid)[0]
                if worker_id not in workeridReqsMap:
                    workeridReqsMap[worker_id]=[]
                workeridReqsMap[worker_id].append(req)
            generator = RandomNumberGenerator(0,self.server_args.detokenizer_worker_num-1)
            for reqs in workeridReqsMap.values():
                detokenizer_index = generator.get_next()
                if self.is_generation:
