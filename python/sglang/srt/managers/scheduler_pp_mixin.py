...
def _ordered_intersection_prefill_ready_views(
    ...
    right_map = {item.rid: item for item in right}
    matched = []
    mismatched = []
    for item in left:
        peer = right_map.get(item.rid)
        if peer is None:
            continue
        if peer == item:
            matched.append(item)
        else:
            mismatched.append((item, peer))

    if mismatched:
        logger.warning(
            "PP prefill ready view mismatch at local consensus: mismatches=%s",
            [
                (
                    left_item.rid,
                    {
                        "upstream_ready_len": left_item.ready_len,
                        "local_ready_len": right_item.ready_len,
                    },
                )
                for left_item, right_item in mismatched
            ],
        )
...
def _prefill_ready_views_to_rids(
    views: Optional[List[PPPrefillReadyView]],
) -> Optional[List[str]]:
    if views is None:
        return None
...
class SchedulerPPMixin:
    ...
    def _pp_get_authoritative_prefill_ready_views(
        ...
        if batch is None:
            return []
        return [
            PPPrefillReadyView(rid=req.rid, ready_len=len(req.prefix_indices))
            for req in batch.reqs
        ]

    def _pp_get_local_prefill_ready_views(
        self: Scheduler,
    ) -> List[PPPrefillReadyView]:
        ...
        if self.chunked_req is not None and self._pp_is_prefill_req_locally_ready(
            ...
            ready_views.append(
                PPPrefillReadyView(
                    ...
                    ready_len=len(self.chunked_req.prefix_indices),
                )
            )
        ...
        for req in self.waiting_queue:
            ...
            if self._pp_is_prefill_req_locally_ready(req):
                ready_views.append(
                    PPPrefillReadyView(
                        rid=req.rid,
                        ready_len=len(req.prefix_indices),
                    )
                )
                seen.add(req.rid)
    ...
    def _pp_pd_get_prefill_ready_views(
        self: Scheduler,
    ) -> List[PPPrefillReadyView]:
    ...
    def _pp_validate_authoritative_prefill_ready_views(
        ...
        if self.pp_group.is_first_rank:
            authoritative_view_map = {
                authoritative_view.rid: authoritative_view
                for authoritative_view in authoritative_views
            }
            mismatched_views = []
            for local_view in local_views:
                authoritative_view = authoritative_view_map.get(local_view.rid)
                if authoritative_view != local_view:
                    mismatched_views.append((authoritative_view, local_view))

            if not mismatched_views:
                return

            logger.error(
                "PP0 selected prefill ready views mismatch before launch at PP%s "
                "ATTN_CP%s TP%s: authoritative=%s local=%s",
                self.pp_rank,
                self.attn_cp_rank,
                self.attn_tp_rank,
            ...
            raise RuntimeError(
                "PP0 selected prefill ready views mismatch before launch: "
                f"pp={self.pp_rank} cp={self.attn_cp_rank} tp={self.attn_tp_rank} "
