...
def _worker_loop( task_queue, tokenizer_config, rid_to_state, state_lock):
    ...
    while True:
        ...
        try:
            ...
            _handle_batch_output2(recv_obj, rid_to_state, stream_output, speculative_algorithm, tokenizer, state_lock)
...
class MultiProcessTaskExecutor:
    def __init__(self, task_queue: mp.Queue, num_workers: int = 4, max_queue_size: int = 4096, tokenizer_config: dict = None, rid_to_state=None, state_lock=None):
    ...
    def start(self):
        for i in range(self.num_workers):
            p = Process(target=_worker_loop, name=f"Worker-{i}", args=(self.task_queue, self.tokenizer_config, self.rid_to_state, self.state_lock))
            p.daemon = True
            p.start()
