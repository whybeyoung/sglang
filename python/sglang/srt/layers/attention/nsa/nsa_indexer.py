...
class Indexer(CustomOp):
    ...
    def _get_q_k_bf16(
        ...
        if cp_input_dict is not None:
            ...
            kv_all = attn_tp_all_gather_reorgan_into_tensor(
                key.contiguous(),
            ...
            outputs_list = torch.split(
                kv_all, cp_input_dict["reverse_split_len"], dim=0
            )
            key = torch.cat(
    ...
    def _get_topk_ragged_with_cp(
        ...
        if cp_index is not None:
            ...
            for i in range(len(cp_index)):
                batch_idx, start_seq_position, end_seq_position = cp_index[i]
                pre_chunk_offset = (
            ...
            topk_result = metadata.topk_transform(
                logits, self.index_topk, actual_seq_q, ke_offset, batch_idx_list
            )
        else:
            kv_len = (
            ...
            topk_result = metadata.topk_transform(
                logits, self.index_topk, actual_seq_q, ke_offset
            )
    ...
    def forward_cuda(
        ...
        query, key = self._get_q_k_bf16(
            q_lora, x, positions, enable_dual_stream, cp_input_dict
        )
        ...
        if is_cuda():
            ...
            if (
                ...
                if cp_input_dict is not None:
                    ...
                    # cp_batch_seq_index_prev = cp_input_dict["cp_batch_seq_index_prev"]
                    # cp_batch_seq_index_next = cp_input_dict["cp_batch_seq_index_next"]
                    cp_batch_seq_index_next = None
                    cp_batch_seq_index_prev = None
