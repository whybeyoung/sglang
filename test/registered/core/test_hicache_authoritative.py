...
class TestHiCacheAuthoritative(CustomTestCase):
    ...
    def test_authoritative_live_match_keeps_backup_boundary_host_hit_without_path_pending(
        self,
    ):
        cache = HiRadixCache.__new__(HiRadixCache)
        ...
        cache.authoritative_backuped_node_ids = set()
        cache.authoritative_host_visible_node_ids = set()
        cache.authoritative_backuped_boundary_keys = {(4, "backup-102")}
        cache.authoritative_host_visible_boundary_keys = set()
        cache.authoritative_prefetch_ready_by_reqid = {
            "rid": types.SimpleNamespace(
                input_len=6,
    ...
    def test_authoritative_live_match_suppresses_backup_boundary_host_hit_with_path_pending(
        self,
    ):
        cache = HiRadixCache.__new__(HiRadixCache)
        ...
        cache.authoritative_backuped_node_ids = set()
        cache.authoritative_host_visible_node_ids = set()
        cache.authoritative_backuped_boundary_keys = {(4, "backup-103")}
        cache.authoritative_host_visible_boundary_keys = set()
        cache.authoritative_prefetch_ready_by_reqid = {
            "rid": types.SimpleNamespace(
                input_len=6,
        ...
        cache.authoritative_host_insert_missing_by_reqid = {"rid": [{"reason": "unresolved"}]}
        cache.authoritative_pending_backup_refs = {}
        cache.authoritative_pending_backup_reasons = {}
        cache._is_authoritative_ready_stable = lambda req_id: False
        cache._clamp_match_result_to_authoritative_summary = (
    ...
    def test_canonicalize_prefetch_ready_result_keeps_usable_request_scoped_host_ready(
        ...
        cache._visible_host_hit_length_to_node = (
            lambda node, include_prefetch_visible=True: 512
        )
    ...
    def test_is_prefetch_ready_result_usable_rejects_incomplete_visible_chain(self):
        cache = HiRadixCache.__new__(HiRadixCache)
        ...
        cache._node_request_ready_visible = lambda node: True
        ...
        self.assertFalse(cache.is_prefetch_ready_result_usable(ready))
    ...
    def test_collect_stable_ready_visible_nodes_requires_matched_prefix_bound(self):
        ...
        partial = cache._collect_stable_ready_visible_nodes(
            ready,
            matched_length=1,
        )

        self.assertEqual([node.id for node in stable], [11, 12])
        self.assertEqual([node.id for node in partial], [12])
    ...
    def test_collect_stable_ready_visible_nodes_rounds_down_to_page_boundary(self):
        cache = HiRadixCache.__new__(HiRadixCache)
        cache.page_size = 4
        ...
        class Node:
            def __init__(self, node_id, parent=None):
                ...
                self.parent = parent
                self.evicted = True
                self.host_value = torch.arange(4, dtype=torch.int64)

        root = object()
        host_1 = Node(31, root)
        host_2 = Node(32, host_1)
        ready = LatchedPrefetchReadyResult(
            match_result=MatchResult(
                device_indices=torch.empty((0,), dtype=torch.int64),
                last_device_node=root,
        ...
        self.assertEqual([node.id for node in stable], [32])
    ...
    def test_match_prefix_suppresses_chunked_revoked_live_match_residue(self):
        ...
        cache.pp_device_only_match_fallback = False
        cache.prefetch_revoked_rids = {"rid-revoked"}
        cache.prefetch_revoked_token_counts = {"rid-revoked": 31424}
        cache._clamp_match_result_to_authoritative_summary = (
            lambda req_id, match_result, input_len=None: match_result
        )
        cache._format_authoritative_resolution_stats = lambda: "none"
        ...
        match_node.parent = root
        cache._match_prefix_helper = (
            lambda node, key: ([torch.arange(512, dtype=torch.int64)], match_node)
        )

        req = types.SimpleNamespace(
            rid="rid-revoked",
            fill_ids=list(range(31981)),
            is_chunked=1,
        )
        result = cache.match_prefix(
            MatchPrefixParams(
                key=RadixKey(token_ids=list(range(512)), extra_key=None),
                req=req,
                cow_mamba=False,
            )
        )
